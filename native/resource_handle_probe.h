...
inline bool probe_native_resource_handles(wi::scene::Scene& scene) {
    ...
    if (!check(resource_registry.destroy(handle_probe), "native handle destruction") ||
    ...
    if (!check(reused_handle.slot == handle_probe.slot && reused_handle.generation != handle_probe.generation,
