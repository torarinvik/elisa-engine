...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Follow-up: `MallocStackLoggingNoCompact` snapshots pointed to cycle-7 Wicked pipeline-cache/Metal setup and 116 per-frame Lua temporary tables (6,496 bytes) across 58 measured restarts. Wicked commit `851c8f2` avoids allocating the timer-wakeup queue when no timer is due; a Lua regression probe verifies 1,000 idle updates stay below 1 KiB heap growth and that due coroutines resume. The 512-cycle uninstrumented soak passed with a 0-byte GPU delta and 5,904-byte heap increase, down from 66,944 bytes before the fix. The complete post-fix SDL3/Wicked gate also passed with exact frame determinism, 0-byte GPU delta and 1,824-byte heap delta over 58 restarts; eight host cycles grew by 22,064 bytes. Instrumented sizes remain invalid leak estimates. F05 stays partial: residual heap and 277,184-byte post-churn deltas still need attribution. Commands and full results: [`docs/validation/orderly-shutdown.md`](docs/validation/orderly-shutdown.md).
...
- [ ] **A09 · P2 · FBX import with ufbx** — After: A05, C01.
  ...
  Progress: ufbx v0.23.0 is pinned by commit and SHA-256 in the dependency manifest. `native/fbx_asset_import.h` performs strict, memory-bounded FBX parsing, summarizes scene/rig/clip/material metadata, normalizes axis and units to right-handed +Y-up metres, and decodes the largest triangle mesh into finite indexed positions/normals/UVs. Its offline import caps are 1.5 GiB temporary and 3 GiB scene memory for dense source geometry. `scripts/cook_fbx_asset.py` writes geometry into `elisa-cooked-v2`, optionally simplifies it through pinned meshoptimizer, and validates source hashes and package bounds; the public `RenderScene::create_mesh` loads a checked package into Wicked. Evidence: the walking mesh cooks to 83,442 triangles, and the supplied Arc Gate simplifies from 3,077,694 to 12,000 triangles (10,009 vertices, 619,538 bytes, 0.00124 relative error); the project runner cooks both before building. Full scene hierarchy, material subsets/textures, skin weights/bind poses, animation tracks and asynchronous asset residency remain.
