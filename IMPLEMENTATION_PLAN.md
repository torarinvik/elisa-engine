...
- [ ] **R01 · P0 · Render extraction and entity mapping** — After: F04, F06, F07.
  ...
  Progress: `src/backend/render_snapshot.elisa` adds a bounded persistent snapshot keyed by world-branded gameplay references and stable render IDs; one gameplay entity can fan out to several instances, updates preserve a row, and removal compacts it. `test/render_snapshot.elisa` and `native/render_snapshot_bridge.h` are gated; the native bridge validates transactional row batches and rolls back newly created entities on staging failure. `src/runtime/render_scene.elisa` exposes a persistent Wicked scene service for primitive creation, transforms, color/visibility, camera control, resize, and generation-checked handles. `RenderScene::sync_snapshot` now reconciles a snapshot into that live scene, reuses stable IDs, updates transforms, and retires removed rows; its native smoke checks the rendered pixels and reconciliation lifecycle. Evidence: [`docs/validation/elisa-render-scene.md`](docs/validation/elisa-render-scene.md). Snapshot rows currently become default boxes and are submitted one at a time; world extraction, asset/material identity, and a transactional native batch path remain. Full scene submission still needs to replace maze-specific manifest construction.
