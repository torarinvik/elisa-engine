...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Progress: `NativeApplication` now owns Wicked through an explicit destruction boundary, closes and drains RAII callback scopes, runs reverse-order shutdown hooks, waits for GPU work, clears the global graphics device, calls the pinned Wicked `wi::audio::Shutdown()` SDL3 lifetime hook, destroys the window, and quits SDL; the native probe now repeats two hidden initialize/shutdown cycles after scene teardown, and the persistent self-test returns normally with `ELISA_ORDERLY_SHUTDOWN=1`. Allocator-pressure accounting and broader service registration remain.
- [ ] **F06 · P0 · Native resource handles and retirement** — After: F04.
  ...
  Progress: `native/resource_handles.h` now provides typed mesh/material/texture/body pools with real Wicked entities and a GPU texture, while `native/voice_handles.h` wraps the real bounded miniaudio voices; the native probe proves logical invalidation, incomplete-fence retention, completed-fence collection, stale/cross-scene rejection, failed creation, slot protection, generation reuse, deterministic generation exhaustion, four rounds of 16-resource create/destroy pressure with object-baseline checks, and allocator telemetry for 69 creations, failures, logical destruction, and retirement collection. A production upload/material binding layer remains.
...
- [ ] **R07 · P1 · Post-processing and quality profiles** — After: R05, F08.
  ...
  Progress: `src/backend/quality.elisa` now defines validated low/medium/high profiles with explicit tonemap, upscaler, bloom, AO, SSR, depth, and render-scale policy; `native/postprocess_bridge.h` applies the queried profile to Wicked and reports unsupported FSR as a fallback. History-resource transition validation, fog policy, and measured GPU/VRAM costs remain.
