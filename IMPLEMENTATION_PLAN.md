...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Progress: `NativeApplication` tracks SDL/Wicked partial initialization and rolls back failures at SDL, window, and Wicked stages; retry and lifecycle counters account for every attempt. Shutdown closes callback admission and drains a concurrent in-flight callback before reverse-order service functions, stops audio while SDL is live, waits for GPU work, destroys the Wicked application, clears the global device, destroys the window, and quits SDL. A fixed 64-entry function/context registry avoids allocation during service registration; the native gate fills it, checks overflow rejection and reverse teardown order, and reports peak callback/hook pressure across repeated hidden lifecycle cycles. Evidence: [`docs/validation/orderly-shutdown.md`](docs/validation/orderly-shutdown.md), two-pass native gate passed. Byte-level accounting of allocations internal to pinned Wicked libraries remains.
...
- [ ] **F07 · P0 · Coordinate and numeric conventions** — After: F04.
  Centralize units, handedness, matrix order, quaternion layout, depth, winding, tangent parity, and camera conversion instead of probe-local sign flips. Done: round trips and an asymmetric scene validate render, physics, skinning, ray picking, and negative/nonuniform scale conventions. Progress: `native/coordinate_abi.h` now validates and converts column-major affine matrices and normalized XYZW quaternions; `native/coordinate_transform_bridge.h` submits TRS and matrix payloads to Wicked with explicit units and row-vector layout conversion, rejecting shear before decomposition. The native probe verifies matrix/quaternion basis equivalence, unit scaling, real Wicked transform round trips, depth/projective/zero-quaternion rejection, and the existing shared render/physics/skin/picking fixture. Evidence: [`docs/validation/coordinate-conventions.md`](docs/validation/coordinate-conventions.md), two-pass native gate passed. Rendered asymmetric-scene reference captures remain.
- [ ] **F08 · P0 · Runtime capability negotiation** — After: F04.
  Replace coarse profile assumptions with queried service/format/limit/feature support and explicit fallback policy. Done: unavailable renderer features and missing optional libraries yield actionable errors or tested fallbacks; a dependency's advertised feature cannot automatically mark the engine capability supported. Progress: Elisa resolves bounded feature and limit requirements into `Ready`, `Fallback`, `Unavailable`, or `Invalid`; `negotiate_limit` includes remaining memory, and typed texture negotiation matches the native BC1/normal-map/alpha fallback rules. ABI v2 in `native/capability_abi.h` carries optional graphics bits, queried RGBA8/BC1/R16F support, memory budget/usage, and actual Wicked high-priority/streaming worker counts. Typed C queries validate individual features, formats, and viewport/worker/memory limits; unknown bits and inconsistent profiles are rejected. Evidence: `test/capabilities.elisa` and `test/maze_bundle.elisa` compiled and passed; [`docs/validation/capability-negotiation.md`](docs/validation/capability-negotiation.md); two-pass native gate passed against the Apple M5. Mapping a live host profile into runtime services and broader service-specific fallback matrices remain.
- [ ] **F09 · P0 · SDL3 lifecycle and frame pacing** — After: F02, F07.
  Handle focus, high-DPI resize, minimize, fullscreen, display changes, close, and suspended simulation; separate fixed ticks from presentation. Done: zero-sized windows and variable render cadence do not corrupt simulation, busy-spin, stretch viewports, or lose input transitions. Progress: `NativeApplication::WindowState` now suspends on focus loss, minimize, or zero pixel extent; `run_frame()` skips Wicked presentation and `advance_fixed()` clears backlog while suspended. The lifecycle probe covers focus/minimize/restore, fullscreen, high-DPI dimensions, zero-extent presentation/tick suppression, and clean fixed-step resume. Evidence: [`docs/validation/sdl3-lifecycle.md`](docs/validation/sdl3-lifecycle.md), two-pass native gate passed. Real swapchain recreation through display-mode changes and gameplay-owned fixed-tick scheduling remain.
