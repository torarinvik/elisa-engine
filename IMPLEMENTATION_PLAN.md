...
- [ ] **F07 · P0 · Coordinate and numeric conventions** — After: F04.
  Centralize units, handedness, matrix order, quaternion layout, depth, winding, tangent parity, and camera conversion instead of probe-local sign flips. Done: round trips and an asymmetric scene validate render, physics, skinning, ray picking, and negative/nonuniform scale conventions. Progress: `native/coordinate_conventions.h` and `native/coordinate_probe.h` now own bidirectional positions/directions, metre scale, ray round trips, asymmetric transforms, and winding parity; matrix/quaternion ABI conversion and captured cross-subsystem reference fixtures remain.
