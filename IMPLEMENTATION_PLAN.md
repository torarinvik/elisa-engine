...
- [ ] **F07 · P0 · Coordinate and numeric conventions** — After: F04.
  Centralize units, handedness, matrix order, quaternion layout, depth, winding, tangent parity, and camera conversion instead of probe-local sign flips. Done: round trips and an asymmetric scene validate render, physics, skinning, ray picking, and negative/nonuniform scale conventions. Progress: `native/coordinate_abi.h` validates/converts column-major affine matrices and normalized XYZW quaternions; `native/coordinate_transform_bridge.h` submits TRS/matrix payloads to Wicked with explicit units and row-vector conversion, rejecting shear. The native probe covers matrix/quaternion basis equivalence, unit scaling, real Wicked transform round trips, and invalid depth/projective/zero-quaternion payloads. A second real render now captures an oblique negative, nonuniform cuboid and asymmetric color markers; the gate checks exact repeat-run pixels and compares against [`backends/coordinate_reference.png`](backends/coordinate_reference.png). Evidence: [`docs/validation/coordinate-conventions.md`](docs/validation/coordinate-conventions.md), two-pass native gate passed. Still open: tangent parity is not yet applied by a production mesh/material path, and physics, skinning, and ray picking need end-to-end signed-scale fixtures rather than shared conversion-only assertions.
