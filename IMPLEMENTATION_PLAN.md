...
- [ ] **A09 · P2 · FBX import with ufbx** — After: A05, C01.
  ...
  Progress: ufbx v0.23.0 is pinned by commit and SHA-256 in the dependency manifest. `native/fbx_asset_import.h` now performs strict, memory-bounded FBX parsing, summarizes scene/rig/clip/material metadata, normalizes axis and units to right-handed +Y-up metres, and can decode the largest triangle mesh into finite indexed positions/normals/UVs. A synthetic cm-unit triangle and the supplied walking/running/fence files pass `python3 scripts/test_fbx_import.py --assets-root <assets>`, including the shared character rig and clip durations. `scripts/wicked_probe.elisascript` runs the synthetic import check in its build gate. Full scene hierarchy, material subsets/textures, skin weights/bind poses, animation tracks, conversion into A05/C01 normalized packages, and Elisa load/render integration remain; this is not yet an asset cooker.
