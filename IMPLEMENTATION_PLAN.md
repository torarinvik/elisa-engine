...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Follow-up: `MallocStackLoggingNoCompact` snapshots point to cycle-7 Wicked pipeline-cache/Metal setup and 116 per-frame Lua temporary tables (6,496 bytes) across 58 measured restarts. Instrumentation itself added ~45.8 MB at cycle 7, so those sizes are not leak estimates; see [`docs/validation/orderly-shutdown.md`](docs/validation/orderly-shutdown.md). Next: run a longer uninstrumented soak after renderer warm-up and Lua collection, and attribute the one-time host/device step.
