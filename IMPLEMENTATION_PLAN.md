...
- [ ] **F09 · P0 · SDL3 lifecycle and frame pacing** — After: F02, F07.
  Handle focus, high-DPI resize, minimize, fullscreen, display changes, close, and suspended simulation; separate fixed ticks from presentation. Done: zero-sized windows and variable render cadence do not corrupt simulation, busy-spin, stretch viewports, or lose input transitions. Progress: `NativeApplication::WindowState` suspends on focus loss, minimize, or zero pixel extent; `run_frame()` skips Wicked presentation and `advance_fixed()` clears backlog while suspended. `native/input_tick_queue.h` keeps movement and restart events in bounded FIFO order until fixed ticks consume them; overflow is explicit, and tests cover event ordering, ring wrap, and the embedded game's scheduled input. The lifecycle probe covers focus/minimize/restore, fullscreen, high-DPI dimensions, zero-extent presentation/tick suppression, and clean fixed-step resume. Evidence: [`docs/validation/sdl3-lifecycle.md`](docs/validation/sdl3-lifecycle.md), two-pass native gate and persistent-host self-test passed. Real Wicked swapchain recreation through a display-mode change remains.
