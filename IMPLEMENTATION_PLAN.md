...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Progress: `NativeApplication` rolls back SDL/Wicked initialization failures at SDL, window, and Wicked stages. Shutdown closes callback admission, runs reverse-order hooks, drains Wicked jobs, then stops audio while SDL is live, waits for GPU work, destroys the Wicked application, clears the global device, destroys the window, and quits SDL. The shutdown-hook test submits a job and verifies it completes before device teardown. A fixed 64-entry function/context registry checks overflow rejection and reverse teardown order. Wicked commit `e45123b` destroys placement-constructed command lists on Metal, Vulkan, and DX12 and adds `WaitForAllJobs()`. Wicked commit `857d170` releases Metal shader dispatch data, the FAudio engine, and its owned reverb effect. The leak report fell from about 1.38 MiB of engine/library allocations to 26 KiB in Apple AppIntents/LinkServices cycles; the separate non-instrumented heap high-water still rises about 1.90 MiB over eight host/device cycles. Evidence: [`docs/validation/orderly-shutdown.md`](docs/validation/orderly-shutdown.md); the full native gate passed after these patches, including two deterministic frame passes and eight host/device shutdown cycles. F05 remains incomplete pending longer-run attribution and in-process scene restart coverage.
