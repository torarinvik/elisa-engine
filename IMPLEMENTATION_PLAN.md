...
- [ ] **A09 · P2 · FBX import with ufbx** — After: A05, C01.
  ...
  Progress: ufbx v0.23.0 is pinned by commit and SHA-256 in the dependency manifest. `native/fbx_asset_import.h` performs strict, memory-bounded FBX parsing, summarizes scene/rig/clip/material metadata, normalizes axis and units to right-handed +Y-up metres, and decodes the largest triangle mesh into finite indexed positions/normals/UVs. `scripts/cook_fbx_asset.py` writes that mesh into the existing `elisa-cooked-v2` geometry package with source hash and bounded data validation; `native/package_load.h` preserves its UV channel in Wicked's first UV set. The synthetic cm-unit triangle import and cook self-tests pass, and the native reader probe verifies an optional UV channel. The optional walking/running/fence asset-tree checks were not run because those FBX files are absent from this checkout. The Wicked build gate runs both synthetic checks. Full scene hierarchy, material subsets/textures, skin weights/bind poses, animation tracks, and Elisa load/render integration remain.
