...
- [ ] **A09 · P2 · FBX import with ufbx** — After: A05, C01.
  ...
  Progress: ufbx v0.23.0 is pinned by commit and SHA-256 in the dependency manifest. `native/fbx_asset_import.h` performs strict, memory-bounded FBX parsing, summarizes scene/rig/clip/material metadata, normalizes axis and units to right-handed +Y-up metres, and decodes the largest triangle mesh into finite indexed positions/normals/UVs. `scripts/cook_fbx_asset.py` writes that mesh into the existing `elisa-cooked-v2` geometry package with source hash and bounded data validation; `native/package_load.h` preserves its UV channel in Wicked's first UV set. The public `RenderScene::create_mesh` API now loads a bounded cooked package into a checked Wicked instance, and `scripts/elisa_build_run.py` cooks project-declared FBX assets before building. The game runner built and cooked the supplied walking mesh (83,442 triangles, 97,679 vertices); the Wicked gate also covers runtime package geometry and UV decoding. Full scene hierarchy, material subsets/textures, skin weights/bind poses, animation tracks, asynchronous asset residency and fence simplification remain.
