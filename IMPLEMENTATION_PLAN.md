...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Progress: `NativeApplication` tracks SDL/Wicked partial initialization and rolls back failures at SDL, window, and Wicked stages; retries pass after each injected failure. Shutdown closes callback admission and drains a concurrent in-flight callback before reverse-order hooks, stops audio while SDL is live, waits for GPU work, destroys the Wicked application, clears the global device, destroys the window, and quits SDL. The native gate still passes two complete frame runs and repeated hidden lifecycle cycles. Evidence: [`docs/validation/orderly-shutdown.md`](docs/validation/orderly-shutdown.md). Allocator-pressure accounting and broader service registration remain.
