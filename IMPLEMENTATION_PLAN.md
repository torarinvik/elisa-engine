...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Latest full-gate repeat after checked application startup and tangent-bearing FBX packages: GPU deltas remained zero; 64-cycle scene heap deltas were 4,256 and 0 bytes, eight-cycle host lifecycle deltas were 14,080 and 15,456 bytes, and post-churn steady-state deltas were 277,184 and 279,744 bytes. Rendered-frame comparison remained exact. Host/churn heap deltas remain unattributed; F05 stays partial. Evidence: [`docs/validation/orderly-shutdown.md`](docs/validation/orderly-shutdown.md).
...
- [ ] **A09 · P2 · FBX import with ufbx** — After: A05, C01.
  ...
  Progress: ufbx v0.23.0 is pinned by commit and SHA-256 in the dependency manifest. `native/fbx_asset_import.h` performs strict, memory-bounded FBX parsing, summarizes scene/rig/clip/material metadata, normalizes axis and units to right-handed +Y-up metres, and decodes the largest triangle mesh into finite indexed positions/normals/UVs. Dense offline imports now have 1.5 GiB temporary, 3 GiB parsed-scene, and 1 GiB extracted-geometry ceilings. `scripts/cook_fbx_asset.py` writes hash-carrying `elisa-cooked-v2` packages with deterministic tangent frames and optionally simplifies them with pinned meshoptimizer via `--max-triangles`; the project runner forwards a validated per-asset limit. `RenderScene::create_mesh` loads checked packages, uploads optional cooked tangents, reverses Elisa front-face winding for Wicked, and the SDL3/Metal smoke renders the synthetic triangle while testing project path containment and handle cleanup. The amazing-labyrinth checkout records an 83,442-triangle walking mesh cook and the Arc Gate reduction recorded under A07; those source assets are absent here, so neither result was rerun. Full scene hierarchy, material subsets/textures, skin weights/bind poses, animation tracks, and asynchronous asset residency remain.
...
- [ ] **R02 · P1 · Mesh/material/instance API** — After: R01, A04.
  ...
  Progress: `src/backend/render_resources.elisa` provides generation-checked instance handles, shared mesh/material asset IDs, transform/visibility/layer updates, reference counting, and an atomic bounded instance-update batch; `test/render_resources.elisa` verifies independent updates, duplicate-batch rejection, and stale-handle rejection. `native/resource_handles.h` now applies material color, visibility, and layer-mask updates through owner/generation-checked Wicked handles, and records batch call/row/rejection telemetry as an ABI traffic measure; the native gate covers two independent rows and duplicate rejection. Public `RenderScene::create_mesh` creates a checked static Wicked mesh instance from normalized cooked positions, normals, UVs, and indices. Its native Metal smoke cooks and renders an FBX triangle, rejects traversal, absolute and symlink-escape paths, and verifies stale-handle cleanup; the upload clears starter-primitive vertex streams, sets the mesh material binding, and converts Elisa front-face winding for Wicked. `RenderScene::set_emissive` and `set_bloom` expose bounded per-instance HDR emission and render-path bloom controls. Shared mesh ownership, material/texture slots, skinned instances, async loading and large-instance throughput remain.
