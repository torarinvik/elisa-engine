...
- [ ] **F06 · P0 · Native resource handles and retirement** — After: F04.
  ...
  Progress: `native/resource_handles.h` now provides typed mesh/material/texture/body pools with real Wicked entities and a GPU texture, while `native/voice_handles.h` wraps real bounded miniaudio voices. Per-registry owner identities reject handles from a second registry wrapping the same scene/service; fixed-capacity retirement queues reject overflow without invalidating a live resource, and voice generation exhaustion stops newly started playback instead of leaking an untracked voice. The native probe also covers incomplete/completed-fence collection, cross-scene rejection, failed creation, slot protection, generation reuse, four rounds of 16-resource create/destroy pressure with object-baseline checks, and allocator telemetry for 69 creations. Evidence: [`docs/validation/native-resource-retirement.md`](docs/validation/native-resource-retirement.md), two-pass Wicked native gate passed. A production upload/material binding layer remains.
