...
- [ ] **F05 · P0 · Orderly startup and shutdown** — After: F04.
  ...
  Progress: `NativeApplication` now owns Wicked through an explicit destruction boundary, waits for GPU work, clears the global graphics device, calls the pinned Wicked `wi::audio::Shutdown()` SDL3 lifetime hook, destroys the window, and quits SDL; the native probe now repeats two hidden initialize/shutdown cycles after scene teardown, and the persistent self-test returns normally with `ELISA_ORDERLY_SHUTDOWN=1`. General worker/callback drain ownership and allocator-pressure accounting remain.
- [ ] **F06 · P0 · Native resource handles and retirement** — After: F04.
  ...
  Progress: `native/resource_handles.h` now provides typed mesh/material/texture/body pools with real Wicked entities and a GPU texture, while `native/voice_handles.h` wraps the real bounded miniaudio voices; the native probe proves logical invalidation, incomplete-fence retention, completed-fence collection, stale/cross-scene rejection, failed creation, slot protection, generation reuse, deterministic generation exhaustion, and four rounds of 16-resource create/destroy pressure with object-baseline checks. A production upload/material binding layer and allocator-pressure telemetry remain.
...
- [ ] **A05 · P1 · Complete glTF scene import** — After: A02, F07.
  ...
  Progress: `src/assets/scene.elisa` defines a bounded normalized scene contract for stable nodes, primitives, cameras, lights, skins, morph metadata, parent-cycle checks, and unsupported-extension rejection; `test/asset_scene.elisa` covers the contract. Native cgltf traversal, animation channels, and device upload remain.
- [ ] **A06 · P1 · Production texture pipeline** — After: A03, F08.
  ...
  Progress: `native/texture_format_policy.h` selects RGBA8, BC1, or R16F from queried capability bits and forces normal maps to an RGBA8-safe path; `native/texture_upload.h` now validates and uploads bounded cooked RGBA KTX1 payloads into real Wicked GPU textures and the gate feeds that resource to the authored goal material. KTX2/Basis runtime transcode, mip/color-space policy, and memory-budget enforcement remain.
...
- [ ] **R02 · P1 · Mesh/material/instance API** — After: R01, A04.
  ...
  Progress: `src/backend/render_resources.elisa` provides generation-checked instance handles, shared mesh/material asset IDs, transform/visibility/layer updates, and reference counting; `test/render_resources.elisa` verifies independent updates and stale-handle rejection. `native/resource_handles.h` now applies material color, visibility, and layer-mask updates through owner/generation-checked Wicked handles, with native gate coverage. Shared mesh upload, material slots, batching, and ABI traffic measurement remain.
- [ ] **R03 · P1 · Cameras and viewports** — After: R01, F09, W02.
  ...
  Progress: `src/backend/camera.elisa` provides validated perspective/orthographic state, high-DPI viewport resizing, TRS-aware picking rays, and clip-plane policy; `test/camera.elisa` covers both projections and resize scale. `native/camera_bridge.h` now creates/removes real Wicked perspective and orthographic views, applies high-DPI dimensions, and resizes projections in the native gate. Camera switching, render targets, frustum culling, and native scene scheduling remain.
- [ ] **R04 · P1 · PBR material workflow** — After: R02, A05, A06.
  ...
  Progress: `native/pbr_material_bridge.h` now validates and applies base/emissive color, metalness, roughness, double-sidedness, and shadow flags through generation-checked Wicked material handles; the native gate covers foreign-handle rejection, unload, and a cooked KTX1 resource reaching the authored goal material. Texture-slot selection, authored glTF material upload, transparency policy, and tangent/color reference captures remain.
