...
- [ ] **F08 · P0 · Runtime capability negotiation** — After: F04.
  ...
  Progress: `Application::backend_profile()` exposes the shared validated SDL3/Wicked startup report to ordinary Elisa applications, with an error union for stopped hosts, wrong-thread calls, and unavailable or invalid reports. `Application::initialize_with_requirements()` negotiates bounded required-service sets before gameplay; it reports declared fallbacks explicitly and rolls back startup on invalid or unavailable requirements. Both the maze host and normal applications use one native device-query helper. The report advertises only engine-owned services; Physics and Audio remain unavailable until public adapters exist. Native application smoke covers ready, fallback-required, unavailable, invalid-count, rollback, profile-limit, and post-shutdown paths. Evidence: [`docs/validation/capability-negotiation.md`](docs/validation/capability-negotiation.md). Aggregated limit/format requirements and service-specific fallback implementations remain.
...
- [ ] **A09 · P2 · FBX import with ufbx** — After: A05, C01.
  ...
  Progress: ufbx v0.23.0 is pinned by commit and SHA-256 in the dependency manifest. `native/fbx_asset_import.h` performs strict, memory-bounded FBX parsing, summarizes scene/rig/clip/material metadata, normalizes axis and units to right-handed +Y-up metres, and decodes the largest triangle mesh into finite indexed positions/normals/UVs. For one skin deformer it now preserves cluster bone names in index order and the top four normalized influences per vertex; influence data participates in vertex deduplication, and skinned meshes cannot be simplified until influences can be remapped. The bounded cooked-package reader validates these optional streams. The supplied walking FBX passes with 34 cluster bones and 97,679 cooked vertices; its package is 11,755,338 bytes. Dense offline imports have 1.5 GiB temporary, 3 GiB parsed-scene, and 1 GiB extracted-geometry ceilings. `scripts/cook_fbx_asset.py` writes hash-carrying `elisa-cooked-v2` packages and optionally simplifies unskinned meshes with pinned meshoptimizer via `--max-triangles`; the project runner forwards a validated per-asset limit. `RenderScene::create_mesh` loads checked packages, reverses Elisa front-face winding for Wicked, and the SDL3/Metal smoke renders a synthetic triangle while checking path containment and handle cleanup. Full scene hierarchy, FBX material discovery, runtime skin upload/bind poses, animation tracks, and asynchronous asset residency remain.
...
- [ ] **R04 · P1 · PBR material workflow** — After: R02, A05, A06.
  ...
  Progress: `RenderScene::set_texture` now loads project-root-confined base-color, normal, packed surface, and emissive maps through Wicked's resource manager, applies block compression, and requests normal-map compression for the normal slot. The native Metal smoke covers valid base/normal assignments plus traversal, absolute-path, and symlink escapes; full material descriptors, authored channel/color captures, generated mesh tangents, and companion-map cooking remain. Evidence: [`docs/validation/render-scene-textures.md`](docs/validation/render-scene-textures.md).
