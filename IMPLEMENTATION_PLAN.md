...
- [ ] **F04 · P0 · Versioned service ABI** — After: F02, F03.
...
- [ ] **F07 · P0 · Coordinate and numeric conventions** — After: F04.
  Centralize units, handedness, matrix order, quaternion layout, depth, winding, tangent parity, and camera conversion instead of probe-local sign flips. Done: round trips and an asymmetric scene validate render, physics, skinning, ray picking, and negative/nonuniform scale conventions.
- [ ] **F08 · P0 · Runtime capability negotiation** — After: F04.
  Replace coarse profile assumptions with queried service/format/limit/feature support and explicit fallback policy. Done: unavailable renderer features and missing optional libraries yield actionable errors or tested fallbacks; a dependency's advertised feature cannot automatically mark the engine capability supported.
- [ ] **F09 · P0 · SDL3 lifecycle and frame pacing** — After: F02, F07.
  Handle focus, high-DPI resize, minimize, fullscreen, display changes, close, and suspended simulation; separate fixed ticks from presentation. Done: zero-sized windows and variable render cadence do not corrupt simulation, busy-spin, stretch viewports, or lose input transitions.
...
- [ ] **N01 · P1 · Recast bake pipeline** — After: A05, F07.
  Promote the Recast probe into deterministic agent-profile-aware navmesh cooking with area annotations and tile metadata. Done: a multi-room/stair/obstacle scene produces cached nav data, useful bake diagnostics, and debug overlays tied to source geometry.
- [ ] **N02 · P1 · Detour runtime queries** — After: N01, A04.
  Add navmesh ownership, nearest-point, path corridor, raycast, reachability, area cost, and off-mesh link queries. Done: an agent follows real navmesh routes; partial/no-path outcomes, query capacity, stale tiles, and invalid endpoints are explicit.
