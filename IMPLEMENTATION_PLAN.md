...
- [ ] **R01 · P0 · Render extraction and entity mapping** — After: F04, F06, F07.
  ...
  Progress: commit `f2266fd` adds `src/runtime/world_rendering.elisa`, which extracts bindings from live `World` entities, validates references, supports one-to-many render IDs, and prunes despawned or replaced-world rows. Portable tests cover identity, updates, invalid bindings, and capacity. `test/render_snapshot.elisa` and `native/render_snapshot_bridge.h` are gated; the native bridge validates transactional row batches and rolls back newly created entities on staging failure. `src/runtime/render_scene.elisa` exposes a persistent Wicked scene service for primitive creation, transforms, color/visibility, camera control, resize, and generation-checked handles. `RenderScene::sync_snapshot` reconciles extracted rows into that live scene, reuses stable IDs, updates transforms, and retires removed rows; the native smoke now spawns actual Elisa world entities and checks rendered pixels and lifecycle. Evidence: [`docs/validation/elisa-render-scene.md`](docs/validation/elisa-render-scene.md). Transforms still live in a separate render binding table, rows become default boxes, and they are submitted one at a time; general transform-component extraction, asset/material identity, and a transactional native batch path remain. Full scene submission still needs to replace maze-specific manifest construction.
- [ ] **R02 · P1 · Mesh/material/instance API** — After: R01, A04.
  ...
  Progress: `src/backend/render_resources.elisa` provides generation-checked instance handles, shared mesh/material asset IDs, transform/visibility/layer updates, reference counting, and an atomic bounded instance-update batch; `test/render_resources.elisa` verifies independent updates, duplicate-batch rejection, and stale-handle rejection. `native/resource_handles.h` now applies material color, visibility, and layer-mask updates through owner/generation-checked Wicked handles, and records batch call/row/rejection telemetry as an ABI traffic measure; the native gate covers two independent rows and duplicate rejection. Public `RenderScene::create_mesh` also creates a checked static Wicked mesh instance from normalized cooked positions, normals, UVs, and indices. Shared mesh ownership, material/texture slots, skinned instances, async loading and large-instance throughput remain.
