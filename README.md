...
asset. The shared cooker emits a compressed runtime package and a raw-stream
Godot companion because the native loader decodes meshoptimizer streams while
Godot's GDScript host does not. Both cooked variants are checked against the
source import and the fixture's pinned triangle count. `scripts/cook_assets.py` bounds untrusted import input (document, buffer,
...
SDL3 is the platform default everywhere (`src/backend/sdl3.elisa`, the gated
SDL3 platform probe, the standalone embedding host, and the Wicked probe).
Wicked is configured with `-DWICKED_USE_SDL3=ON` and links directly to SDL3.
...
object or marker moves to the queried player cell, and a second verified frame
is captured (`native/live_game_probe.h`, `scripts/build_godot_extension.py`,
`backends/godot/capture.gd`).
...
`src/tooling/inspector.elisa` plus `src/tooling/inspector_view.elisa`,
