#!/usr/bin/env python3
"""Measure the bounded large-tree Elisa hierarchy across fresh processes."""
...
def main() -> int:
    ...
    if not compiler or not Path(compiler).is_file():
        ...
        return 2
    binary = ROOT / "build/hierarchy-benchmark"
    binary.parent.mkdir(parents=True, exist_ok=True)
    compile_result = subprocess.run(
        [compiler, "-emit", "exe", "-o", str(binary), str(ROOT / "test/hierarchy_benchmark.elisa")],
    ...
    for _ in range(args.runs):
        ...
        if result.returncode != 0:
            print(f"hierarchy benchmark workload failed with {result.returncode}", file=sys.stderr)
            return result.returncode
        samples_ms.append(elapsed_ms)
    ordered = sorted(samples_ms)
    p95 = ordered[min(len(ordered) - 1, int(len(ordered) * 0.95))]
    print("hierarchy benchmark: "
