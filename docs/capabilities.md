...
| Bounded native collision queries | Tested + Implemented | `native/physics_query_bridge.h`, `native/physics_contact_bridge.h`, `src/physics/queries.elisa`, `docs/validation/physics-queries.md`; nearest/all-hit rays, sphere/capsule casts and overlaps, collision layers, sensors, fixed-capacity contact events, destroyed participants, and stale query tokens are gated; broader cross-service query policy remains P03 |
