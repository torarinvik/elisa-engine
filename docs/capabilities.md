...
| Bounded runtime package index and virtual-path validation | Tested + Implemented | `native/virtual_package.h`, `native/package_manifest.h`, `native/virtual_file_service.h`, `native/package_load.h`, `native/package_bounds_probe.h`; text packages and version-1 `ELPK` indexes validate, bounded zstd sections decompress with CRC-32 verification, manifest dependencies validate in prerequisite-first order, and coalesced reads perform filesystem/package work outside the service lock and drop cancelled or stale-generation results before publication; traversal, symlink escapes, cycles, duplicate/overlapping sections, malformed lines, unsupported compression, and missing files are rejected |
