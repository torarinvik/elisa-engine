...
| Context-aware action bindings with dead zones, chords, edge transitions, and disconnect cleanup | Tested + Implemented | `src/runtime/action_input.elisa`, `test/action_input.elisa`, `native/action_input_bridge.h`, `scripts/wicked_probe.elisascript`, [action-input validation](validation/action-input.md) |
...
| Versioned backend profiles with queried limits and feature requirements | Tested + Implemented | `src/backend/capabilities.elisa`, `native/capability_abi.h`, `native/capability_probe.h`, `examples/maze/capi.elisa`, `test/capabilities.elisa`, [`capability negotiation`](validation/capability-negotiation.md) |
...
| Typed Physics and Audio fallback selection with adapter partial-startup cleanup and retry | Tested + Partial | [`service rollback validation`](validation/runtime-service-rollback.md); provider identity survives failure/re-negotiation, while live provider availability and route orchestration remain caller-owned |
...
| SDL3/Wicked partial-startup rollback, callback drain, and Lua application-handle teardown | Tested + Partial | `native/native_application.h`, `native/window_lifecycle_probe.h`, Wicked `wi::lua::ShutdownApplication`, and [`orderly-shutdown validation`](validation/orderly-shutdown.md); residual host/churn heap changes remain under F05 |
...
| Bounded runtime package index and virtual-path validation | Tested + Implemented | `native/virtual_package.h`, `native/package_manifest.h`, `native/virtual_file_service.h`, `native/package_load.h`, `native/package_bounds_probe.h`; text packages and version-1 `ELPK` indexes validate, bounded zstd sections decompress with CRC-32 verification, manifest dependencies validate in prerequisite-first order, and a fixed worker pool performs filesystem/package work outside the service lock and drops cancelled or stale-generation results before publication; worker shutdown joins before state release, while traversal, symlink escapes, cycles, duplicate/overlapping sections, malformed lines, unsupported compression, and missing files are rejected; the native maze loads its staged bundle outside the checkout under a sandbox that denies the checkout (`docs/validation/packaged-loading.md`); PNG, JPEG and bounded 2D Basis KTX2 bundle sections register as CRC-checked snapshot textures; the maze wall uses one (`docs/validation/bundle-textures.md`); projects declare bundle dependencies, and mesh and texture loads reject a missing, cyclic, escaping, corrupted or over-long dependency closure before reading a section; zstd bombs fail the declared-size bound (`docs/validation/bundle-dependencies.md`) |
...
| Declared read/write sets, derived execution waves | Tested + Implemented | `src/runtime/schedule.elisa`, `src/runtime/executor.elisa`, `test/schedule.elisa`, `native/parallel_executor.h`, [parallel executor validation](validation/parallel-executor.md) |
...
| Generation/owner-checked handles with fence-delayed retirement queue | Tested + Implemented | `native/resource_handles.h`, `native/voice_handles.h`, `native/resource_handle_probe.h`, native Wicked probe; mesh/material/texture/body and miniaudio voice families invalidate logically before serial-gated collection, with repeated resource-pressure baseline checks |
| Elisa-owned Jolt simulation enable/pause boundary | Tested | `native/physics_policy_probe.h`, native Wicked gate, and `Physics::StepClock` in `src/physics/policy.elisa`; paused simulation holds a real dynamic body and portable ticks commit contiguously, while native tick driving remains P01 work |
...
| Persistent one-to-many render extraction and transactional Wicked sync | Tested + Implemented | `src/backend/render_snapshot.elisa`, `src/runtime/world_rendering.elisa`, `src/runtime/render_snapshot_scene.elisa`, `test/render_snapshot.elisa`, `native/render_scene_snapshot_abi.inc`, `docs/validation/elisa-render-scene.md`; snapshot rows carry gameplay and mesh/material identity plus a validated per-instance color/emissive tint (`docs/validation/instance-tints.md`), while cooked-asset resolution remains A03/A04 |
...
| Deferred structural world command buffer | Tested | `src/world/commands.elisa`, `src/world/access.elisa`, `src/world/phase_commands.elisa`, `test/world_commands.elisa`, `test/world_command_primary.elisa`, `docs/validation/deferred-world-commands.md`; affine phase conflicts and exclusive commits are gated; compiler phase-borrow diagnostics and event scheduling remain W03 |
| Versioned prefab instances with runtime remapping | Tested + Implemented | `src/world/prefab.elisa`, `src/world/prefab_world.elisa`, `src/world/prefab_persistence.elisa`, `src/world/prefab_scene.elisa`, `scripts/scene_file.py`, `test/prefab.elisa`, `test/prefab_world.elisa`, `test/prefab_scene.elisa`, `docs/validation/prefab-instances.md`; canonical durable scene files rebuild fresh runtime bindings; native world epochs and resource rehydration remain |
| Generation-checked world-cell streaming | Tested | `src/world/cell_streaming.elisa`, `src/world/cell_world.elisa`, `test/cell_streaming.elisa`, `test/cell_world.elisa`, `docs/validation/world-cell-streaming.md`; native decode/upload scheduling and scene payload activation remain W05 |
| Versioned transactional save records | Tested + Implemented | `src/world/save_schema.elisa`, `test/save_schema.elisa`, `scripts/save_journal.py`, `docs/validation/save-schema.md`; journaled atomic replacement, corrupt-journal fallback, and malformed/truncated-payload diagnostics are gated |
| Application-owned local user data from Elisa | Tested + Implemented | `src/runtime/user_data.elisa`, `native/user_data_abi.cpp`, `test/user_data_probe.elisa`, `test/user_data_service_test.cpp`, `docs/validation/user-data.md`; bounded versioned blobs, platform paths, atomic replacement, and recoverable errors |
| Persistent render-quality profiles from Elisa | Tested + Implemented | `src/runtime/quality_settings.elisa`, `src/backend/quality.elisa`, `test/user_data_probe.elisa`, `test/quality_settings_native_main.elisa`, `docs/validation/quality-settings.md`; stable versioned enum and fixed-point float fields stored through application-owned user data |
| Typed world event queue and service phases | Tested | `src/world/events.elisa`, `test/world_events.elisa`, `docs/validation/world-events.md`; branded entity references reject despawned or stale-epoch events before queueing; native callback delivery remains W08 |
| Native worker-to-main world-event handoff | Tested + Implemented | `native/world_event_bridge.h`, `scripts/wicked_probe.elisascript`, `docs/validation/world-events-native.md`; bounded phase-gated delivery, unsubscribe suppression, and reentrant dispatch/phase guards are gated |
| Shared mesh/material render instances | Tested + Implemented | `src/backend/render_resources.elisa`, `test/render_resources.elisa`, `native/resource_handles.h`, `native/resource_handle_probe.h`, `docs/validation/render-resources.md`; native instances update material color, visibility, and layer masks behind generation-checked handles |
| Perspective and orthographic camera rays | Tested + Implemented | `src/backend/camera.elisa`, `src/runtime/render_scene.elisa`, `test/camera.elisa`, `test/render_scene_camera_native.elisa`, `native/camera_bridge.h`, `scripts/wicked_probe.elisascript`, `docs/validation/cameras-viewports.md`; native high-DPI projection creation, resize, and active-camera ray unprojection are gated |
| Generation-checked Wicked lights and environment probes | Tested + Implemented | `native/lighting_bridge.h`, `scripts/wicked_probe.elisascript`, `docs/validation/lighting-service.md`; directional/point/spot descriptors, shadow flags, realtime probes, validation, and unload are gated |
| Native PBR material updates | Tested + Implemented | `native/pbr_material_bridge.h`, `scripts/wicked_probe.elisascript`, `docs/validation/pbr-materials-native.md`; validated base/emissive color, metalness, roughness, opaque/mask/blend alpha policy, base-color texture slot, double-sidedness, shadow flags, foreign handles, and unload are gated |
| Cooked RGBA KTX1 GPU upload | Tested + Implemented | `native/texture_upload.h`, `scripts/wicked_probe.elisascript`, `docs/validation/texture-upload.md`; bounded KTX validation and real Wicked texture creation feed the authored goal material |
| Normalized native glTF scene traversal | Tested + Implemented | `native/asset_import.h`, `scripts/wicked_probe.elisascript`, `docs/validation/gltf-scene-import.md`; cgltf node/primitive/camera/light/skin/animation/morph counts, PBR factors/alpha metadata, Basis texture extensions, and unsupported-content rejection are gated |
| Generation-checked physics body and shape lifecycle | Partial + Implemented | `src/physics/runtime.elisa`, `src/physics/bodies.elisa`, `src/physics/queries.elisa`, `native/physics_body_bridge.h`, `native/physics_reusable_shape_abi.inc`, `scripts/cook_physics_collision.py`, `docs/validation/physics-primitive-bodies.md`; reusable primitives, meshes, compounds, materials, velocity, inertia, pose, and bounded collision-only glTF packages are integrated; serialized Jolt shape caching and broader character/constraint paths remain P02/P05/P06 |
| Wicked/Jolt character movement and shape control | Partial + Implemented | `src/physics/runtime_character_api.inc`, `src/runtime/services_character_api.inc`, `native/physics_character_abi.inc`, `test/physics_character_probe.elisa`; grounding, ramp ascent, jump, crouch clearance, moving-platform velocity, fixed-tick cadence, and enabled/disabled collision-layer pairs pass in the SDL3/Metal application smoke; steps, corners, and the interactive obstacle course remain P05 |
| Bounded native collision queries | Tested + Implemented | `native/physics_query_bridge.h`, `native/physics_contact_bridge.h`, `src/physics/queries.elisa`, `docs/validation/physics-queries.md`; nearest/all-hit rays, backend-neutral scene-object/collider/body ray filters, sphere/capsule casts and overlaps, collision layers, sensors, fixed-capacity contact events, destroyed participants, and stale query tokens are covered; broader cross-service policy coverage remains P03 |
| Normalized glTF scene contract and runtime import | Tested + Implemented | `src/assets/scene.elisa`, `native/asset_import.h`, `scripts/cook_gltf_asset.py`, `src/runtime/render_scene_imported_scene.elisa`, `test/asset_scene.elisa`, `docs/validation/gltf-scene-import.md`; bounded node, mesh, material, camera, light, skin, morph, and animation content reaches Wicked; unsupported content fails during cook/load |
...
| Tick-stamped replay divergence diagnostics | Tested + Implemented | `src/runtime/replay.elisa`, `native/replay_trace.h`, `test/replay.elisa`, `docs/validation/replay-diagnostics.md`; bounded little-endian frame encoding, deterministic scalar digest mixing, atomic trace replacement, checksum validation, and corruption rejection are gated; concrete subsystem snapshot wiring remains W09 |
...
| Native scoped 3D debug drawing | Tested + Implemented | `RenderScene::debug_box`, `debug_line`, `debug_text`, `debug_flush`, and `debug_clear` queue bounded owner-thread commands through `native/debug_draw_bridge.h`; the SDL3/Metal/Wicked smoke covers copied text, depth flags, invalid bounds, capacity handling, and render-path flushing in group 231 |
| Native entity picking and selection outlines | Tested + Implemented | `RenderScene::set_pick_identity`, `pick`, `select`, and `clear_selection` keep gameplay references in Elisa while `native/picking_bridge.h` and `native/selection_outline_bridge.h` resolve real Wicked entities/materials; group 232 covers a transformed hit, miss, invalid ray, outline enable, and outline clear |
...
| KTX2/Basis supercompression | Tested + Implemented | `scripts/fetch_basisu.py` (pinned 99f52d63), cooker writes `maze_tile_tex.ktx2`, `native/ktx2_upload.h` selects device-supported BC1/BC3/BC5/BC7 or RGBA8 for bounded 2D mip chains and cubemaps; HDR BC6H/RGBA16F coverage is in the next row; Godot loads LDR KTX2 with a compressed-or-decoded fallback |
| HDR Basis KTX2 textures | Tested + Implemented | `scripts/ktx2_container.py`, `native/ktx2_upload.h`, `native/ktx2_upload_probe.h`, `scripts/basisu_probe.py`, `docs/validation/ktx2-hdr.md`; bounded image cooking accepts the supported UASTC HDR 4x4 profile, and device-selected BC6H with an RGBA16F fallback preserves values above 1.0; GPU evidence is macOS Metal only |
| Portable and native asynchronous asset-loader contract | Tested | `src/assets/loader.elisa`, `test/asset_loader.elisa`, `native/native_resource_loader.h`; request coalescing, staged decode/upload, cancellation, placeholders, generations, resident release, retryable slots, bounded native texture upload, and telemetry are covered; production decode and GPU residency budgets remain A04 |
| Asynchronous snapshot mesh and bundle-texture loading | Tested + Implemented | `native/snapshot_asset_worker.h`, `native/render_scene_snapshot_asset_request_abi.inc`, `src/runtime/render_scene_assets.elisa`, `test/render_scene_async_asset_native.elisa`, `docs/validation/async-snapshot-assets.md`; requests read and decode bundle images on a worker thread, pumps adopt on the owner thread, rows naming a loading asset fail with `AssetPending`, and every cancellation point is tested natively and under ThreadSanitizer; the maze loads its bundles this way; priority scheduling and budget-based eviction remain A04 |
| glTF multi-primitive meshes with per-subset material sets | Tested + Implemented | `scripts/cook_gltf_geometry.py`, `native/cooked_geometry_package.h`, `native/render_scene_snapshot_material_set_abi.inc`, `src/runtime/render_scene_assets.elisa`, `scripts/test_geometry_subsets.py`, `test/render_scene_material_subset_native.elisa`, `docs/validation/gltf-material-subsets.md`; up to 16 primitives cook into index subsets bound to up to 16 material slots, the loader requires an exact in-order partition for static and skinned packages, and snapshot rows name a material set that draws each subset with its slot's material, checked by rendered strip colors and skinned subset probes |
| Cooked glTF slot material factors | Tested + Implemented | `scripts/cook_gltf_geometry.py`, `native/cooked_geometry_package.h`, `native/render_scene_snapshot_material_internal.inc`, `native/render_scene_snapshot_mesh_material_abi.inc`, `src/runtime/render_scene_assets.elisa`, `test/render_scene_cooked_material_native.elisa`, `docs/validation/cooked-slot-materials.md`; each slot's glTF base color, metallic, roughness, emissive, opaque/mask/blend alpha and sidedness cook into bounded records that register as materials or a whole-mesh set with rollback; textures and masks as below |
| Cooked glTF material textures and alpha masks | Tested + Implemented | `scripts/cook_gltf_textures.py`, `scripts/gltf_texture_self_test.py`, `native/cooked_slot_materials.h`, `native/render_scene_snapshot_mesh_material_abi.inc`, `src/runtime/render_scene_assets.elisa`, `test/render_scene_cooked_texture_native.elisa`, `docs/validation/cooked-material-textures.md`; embedded base-color, normal, metallic-roughness (with shared occlusion) and emissive images cook into the mesh's `.elpk` with per-slot references, register from the mesh's own unchanged bundle, and bind to each slot's Wicked material, checked by the rendered cutout, lit and emissive strips; external images, texture transforms, second UV sets and separate occlusion images fail visibly |
| Cooked glTF skin helper hierarchies | Tested + Implemented | `scripts/cook_gltf_skin.py`, `native/cooked_geometry_limits.h`, `native/render_cooked_mesh.h`, `docs/validation/gltf-skin-hierarchies.md`; transformed or animated helpers, sibling and separate skeleton branches, multiple skins, palette remapping, and mixed static/skinned placements remain in the bounded runtime rig; matrix TRS, shear rejection, and live Wicked pose submission are covered |
| Baked static glTF node hierarchies | Tested + Implemented | `scripts/cook_gltf_nodes.py`, `scripts/cook_gltf_geometry.py`, `scripts/gltf_hierarchy_self_test.py`, `test/fixtures/node_hierarchy_panel.gltf`, `test/render_scene_node_hierarchy_native.elisa`, `docs/validation/gltf-node-hierarchies.md`; up to 256 matrix or TRS nodes placing several meshes, repeated placements and mirroring scales bake into one cooked mesh with merged material subsets, checked by rendered strip colors through single-sided materials; the static-bake path rejects skinned/animated nodes for handling by the specialized scene cooker |
| Public imported-scene child queries | Tested + Implemented | `src/runtime/render_scene_imported_scene.elisa`, `native/render_scene_imported_scene_abi.inc`, `test/render_scene_animation_submission_native.elisa`, `docs/validation/gltf-node-hierarchies.md`; bounded mesh/camera/light counts, opaque imported-camera activation, root/child mesh visibility and transform edits, and imported-light shadow toggles preserve generation checks without exposing Wicked entities |
| Shared-mesh instances on the direct scene path | Tested + Implemented | `src/runtime/render_scene_instancing.elisa`, `native/render_scene_mesh_abi.inc`, `test/render_scene_mesh_instance_native.elisa`, `docs/validation/elisa-render-scene.md`; `create_mesh_instance` draws a static source's mesh and material again as a Wicked object naming the source's mesh entity with a per-instance color multiplier, so repeated props load and upload once; clones carry no material, skinned, animated, morphing and cloned sources are rejected, and a source destroyed first keeps its mesh until the last clone goes |
| Per-instance shadow control | Tested + Implemented | `src/runtime/render_scene_shadow.elisa`, `native/render_scene_material_abi.inc`, `test/render_scene_mesh_instance_native.elisa`, `docs/validation/elisa-render-scene.md`; `set_cast_shadow` updates both Wicked object and material flags through a generation-checked public handle and marks the material dirty |
| Eased, phase-matched clip transitions | Tested + Implemented | `src/runtime/render_scene_animation_blend.elisa`, `native/render_scene_animation_abi.inc`, `native/render_scene_animation_internal.inc`, `test/render_scene_animation_blend_native.elisa`, `docs/validation/elisa-render-scene.md`; `play_animation_blended` fades with a smoothstep, starts the next looping clip at the old cycle's phase and keeps a re-requested clip running, and `set_animation_speed` retimes a cycle in place, so accelerating locomotion never restarts its stride |
