...
| Bounded runtime package index and virtual-path validation | Tested | `native/virtual_package.h`, `native/package_load.h`, `native/package_bounds_probe.h`; real cooked packages load while traversal, duplicate sections, malformed lines, and missing files are rejected |
