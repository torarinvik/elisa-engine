...
| Bounded runtime package index and virtual-path validation | Tested + Implemented | `native/virtual_package.h`, `native/virtual_file_service.h`, `native/package_load.h`, `native/package_bounds_probe.h`; text packages and version-1 `ELPK` indexes validate, bounded zstd sections decompress with CRC-32 verification, and coalesced reads honor cancellation and mount generations while traversal, symlink escapes, duplicate/overlapping sections, malformed lines, unsupported compression, and missing files are rejected |
