...
| Cooked glTF skin helper hierarchies | Partial + Tested | `scripts/cook_gltf_skin.py`, `native/cooked_geometry_limits.h`, `native/render_cooked_mesh.h`, `docs/validation/gltf-skin-hierarchies.md`; transformed or animated helper nodes below the skinned mesh-space root remain in the runtime rig while palette indices name only actual joints; matrix TRS, shared-root cancellation, 65-node loading and a live Wicked hierarchy probe are covered; multiple skins and independent skeleton branches remain |
| Baked static glTF node hierarchies | Tested + Implemented | `scripts/cook_gltf_nodes.py`, `scripts/cook_gltf_geometry.py`, `scripts/gltf_hierarchy_self_test.py`, `test/fixtures/node_hierarchy_panel.gltf`, `test/render_scene_node_hierarchy_native.elisa`, `docs/validation/gltf-node-hierarchies.md`; up to 256 matrix or TRS nodes placing several meshes, repeated placements and mirroring scales bake into one cooked mesh with merged material subsets, checked by rendered strip colors through single-sided materials; skins, animation, cameras and lights still fail visibly |
