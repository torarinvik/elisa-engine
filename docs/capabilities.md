...
| Bidirectional coordinate, transform-ABI, ray, asymmetric-scale, and winding-parity checks | Tested + Implemented | `native/coordinate_conventions.h`, `native/coordinate_abi.h`, `native/coordinate_probe.h`, native gate output `coordinates:`; render/physics ownership remains F07 |
...
| Native graphics capability and memory query with explicit optional fallbacks | Tested + Implemented | `native/capability_abi.h`, `native/capability_probe.h`, Wicked `GraphicsDevice::CheckCapability`, memory/viewport queries, and `ELISA_FORCE_OPTIONAL_FALLBACK=1` policy path in the native gate |
...
| Bounded runtime package index and virtual-path validation | Tested + Implemented | `native/virtual_package.h`, `native/package_load.h`, `native/package_bounds_probe.h`; text packages and version-1 `ELPK` indexes validate and bounded zstd sections decompress while traversal, duplicate/overlapping sections, malformed lines, unsupported compression, and missing files are rejected |
