...
|---|---|---|---|---|
| WickedEngine | sibling checkout `b7dcda7931f2f596b46e9f3a832d41fe969130bb`; includes command-list/job-drain fixes, Metal dispatch-data and FAudio releases, Lua application-handle cleanup, a runtime trail queue independent of debug drawing, and optional trail depth testing | `scripts/wicked_probe.elisascript`, `native/wicked_probe.cpp`, `native/render_scene_abi.cpp` | Probe | Native backend owner; F05 still has small process-heap changes and macOS framework allocations across repeated device cycles. |
...
| ufbx | v0.23.0 at commit `fcc5d6ba444cfd3eb80677dba5e37e493941abe5`; `native/fbx_asset_import.h`, `native/fbx_asset_cooker.cpp` | Bounded mesh import, `elisa-cooked-v2` output, and optional UV consumption through `native/package_load.h`; synthetic checks run in the Wicked build gate | Adapter | Asset owner; A09 still needs full normalized hierarchy, materials/textures, skin/animation conversion, and Elisa load/render integration. |
