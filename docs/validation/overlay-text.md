...
signed-integer formatting, position updates, visibility changes, and
destruction. It exposes no Wicked font object or pointer. Text is submitted to
the persistent Wicked `RenderPath3D` 2D pass and uses the scene's logical canvas
coordinates.
...
integer updates, bounded position changes, visibility changes, rendered 2D
pixels, destruction, stale handles, and invalidation after scene replacement.
The pixel readback probe is compiled only by the native smoke; ordinary game
builds do not include its blocking GPU readback.
...
- The same `scripts/render_scene_native_smoke.py` command passed on macOS with
  SDL3, Metal, and Wicked. Its Elisa native test moved an existing label and
  rejected a position outside the viewport coordinate bound; the smoke also
  rendered overlay pixels and ran the Elisa-owned maze sample.
- `python3 scripts/check_source_length.py` and `python3 scripts/check_module_hygiene.py` passed.
- `python3 scripts/check_dependency_manifest.py` still reports baseline drift
  in this isolated checkout: the manifest expects Wicked `c1c93008`, while the
  local arc-enabled checkout is `b7dcda79`, and several optional dependency
  directories are absent. This does not affect the render-scene smoke, which
  builds against the local SDL3/Metal libraries.
