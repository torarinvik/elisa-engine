...
30,000 to 16,176 bytes.

`--lod-ratios 0.5,0.25` now cooks a full-detail package followed by each
descending ratio. Given `--output assets/prop.pkg`, it writes sibling
content-addressed packages and `assets/prop.lod.json`. The manifest records the
source and package SHA-256 values, byte size, triangle and vertex counts,
attribute bytes, and meshoptimizer's measured relative geometric error. Its
`relative_error_budget` is the cumulative maximum across levels, so it remains
monotonic even if independently simplified variants measure out of order. The
manifest is published atomically after every referenced package is ready. The
error is normalized geometric error, not projected screen pixels; camera-based
screen-error conversion and runtime manifest loading/selection remain open.
...
- `DEVELOPER_DIR=/Library/Developer/CommandLineTools /opt/homebrew/bin/python3 scripts/cook_gltf_asset.py --self-test` passed, including deterministic three-level package and manifest checks. Its grid reduced 2,304 triangles to 576 across three variants.
- `DEVELOPER_DIR=/Library/Developer/CommandLineTools /opt/homebrew/bin/python3 scripts/test_geometry_subsets.py` passed all 104 sanitized package-loader cases, including production-reader loads of all three LOD-chain packages and compacted per-placement index bounds.
- `DEVELOPER_DIR=/Library/Developer/CommandLineTools /opt/homebrew/bin/python3 scripts/test_glb_static_cook.py` passed: the bounded GLB pipeline cooked 3,042 source triangles into 55 triangles and 37 vertices.
...
- The full shared gate passed before the multi-level manifest addition; the cooker self-test and 104-case native loader suite above validate the latest LOD chain change. The shared gate still does not perform visual LOD selection, which remains unverified.
