...
- The SDL3/Metal probe records timestamp queries around Wicked's `RenderPath3D::Render()` command lists, separately from end-to-end completed-frame latency. Five warmups plus 21 samples measured GPU medians of 3,073 µs at level 0 and 2,990 µs at level 2 (p95 3,096 / 3,002 µs); completed-frame medians were 8,505 / 8,482 µs. The 83 µs GPU difference is measurable in this run but small; repeat and non-Metal device measurements remain open. Meshoptimizer stream compression remains to be integrated. Run with `DEVELOPER_DIR=/Library/Developer/CommandLineTools ELISA_RENDER_SCENE_RENDER_ONLY=1` plus the compiler/runtime environment shown above and `scripts/render_scene_native_smoke.py`.
