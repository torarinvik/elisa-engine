...
- The native LOD visual regression now renders the centered curved-grid fixture from the same camera pose at level 0 and the default one-pixel-budget level 2. It hides unrelated scene instances and screen-space overlays, waits for Wicked object pipelines, captures both presented frames, and checks that the target mesh is visible before comparing its crop. The fixture's triangle winding now faces the +Z camera. `scripts/compare_renders.py lod-quality` reported a 0.00292 mean maximum-channel error, 0.0196 p95, and 6.28% changed pixels above 2/255 in a 160x294 mesh crop; the images retain the same rectangular silhouette and material region. Capture files are `build/render-scene-lod-fine.png` and `build/render-scene-lod-coarse.png`.
- The same SDL3/Metal run measured 21 completed frames after five warmups per level, waiting for GPU completion after each present: level 0 was 8,355 µs median / 9,370 µs p95; level 2 was 8,217 µs / 9,304 µs. This is whole-frame latency (CPU submission plus GPU completion), not an isolated GPU timestamp. The 138 µs median difference is too small to claim a rendering-cost win from this sample; GPU-only instrumentation and non-Metal measurements remain open. Run with `DEVELOPER_DIR=/Library/Developer/CommandLineTools ELISA_RENDER_SCENE_RENDER_ONLY=1` plus the compiler/runtime environment shown above and `scripts/render_scene_native_smoke.py`.
