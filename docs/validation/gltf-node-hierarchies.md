...
- **Placement ranges.** Direct `create_mesh` imports upload independent
  ...
  skinned mesh nodes as required by glTF, retains transformed non-joint
  ancestors in the rig, and combines multiple skins plus mixed static/skinned
  placements within the package limits. See
