...
determinism, and frame-time checks. The zero extent is injected at the host
boundary because the hidden Metal test window cannot reliably request a true
zero-sized drawable. Wicked swapchain recreation after a real display-mode
change, and gameplay-owned fixed-tick scheduling, remain open.
